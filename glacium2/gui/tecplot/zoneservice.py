...
class ZoneService:
    ...
    def extract_zones(obj: pv.DataSet | pv.MultiBlock) -> list[ZoneItem]:
        zones: list[ZoneItem] = []
        for i, (label, dataset) in enumerate(ZoneService._iter_datasets_with_labels(obj)):
    ...
    def scalar_names_for_active(state: ViewerState) -> list[str]:
        ...
        for idx in state.active_indices:
            for name in ZoneService._scalar_names(state.zones[idx].dataset):
    ...
    @staticmethod
    def _iter_datasets_with_labels(
        obj: pv.DataSet | pv.MultiBlock, prefix: str = ""
    ) -> Iterable[tuple[str, pv.DataSet]]:
            ...
            if keys:
                for key in keys:
                    ...
                    if block is None:
                        continue
                    name = f"{prefix}{key}"
                    yield from ZoneService._iter_datasets_with_labels(block, prefix=name + " / ")
            else:
    ...
    @staticmethod
    def _scalar_names(dataset: pv.DataSet) -> list[str]:
        names: list[str] = []
        try:
