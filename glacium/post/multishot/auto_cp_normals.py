...
# -*- coding: utf-8 -*-
"""
auto_cp_normals.py (multi-format, multi-size)
...
- Saves each plot in multiple sizes (SIZES) and formats (FORMATS).
"""
    ...
    if p.suffix.lower() == ".zip":
        with zipfile.ZipFile(p, "r") as zf:
            ...
            pref = [
                n
                for n in names
                if _re.search(r"soln\.fensap\.\d+\.dat$", Path(n).name, _re.I)
            ]
...
def _read_zone_data(
    lines: List[str], start: int, end: int, nvars: int
    ...
    while k < end and len(floats) < target:
        ...
        if s:
            ...
            for t in s.split():
                ...
                except:  # noqa: E722 - robust parsing
                    pass
        k += 1
    if len(floats) < target:
...
def parse_first_zone(path: Path) -> Tuple[np.ndarray, List[str], Optional[np.ndarray], Dict]:
    ...
    if not zones:
        raise ValueError("Keine ZONEs gefunden")
    s, e = zones[0]
...
def _infer_inlet(
    lines: List[str], var_names: List[str], inlet_name: str = "INLET"
) -> Dict[str, float]:
    import re as _re
    ...
    for s, e in z_ranges:
        ...
        for look in range(s + 1, min(e, s + 12)):
            ...
            if nxt.startswith('"') or _num_line_re.match(nxt):
                break
            header_ext += " " + nxt
        if pat.search(header_ext):
            ...
            break
    if not pick:
        raise ValueError(f"INLET-Zone '{inlet_name}' nicht gefunden")
    nvars = len(var_names)
    nodes, _ = _read_zone_data(lines, pick[0], pick[1], nvars)

    xi = _find_var_fuzzy(var_names, "x")
    if xi is None:
        raise ValueError("x nicht gefunden im INLET")
    x = nodes[:, xi]
    ...
    qi = _find_var_fuzzy(
        var_names, "q", "q_inf", "dynamicpressure", "dynpressure", "dynpress"
    )
...
def compute_cp_on_wall(
    ...
    if pi is None:
        raise ValueError("p nicht gefunden und Cp-Variable nicht verfügbar")
...
def _save_current_figure(base: Path, label: str, dpi: int):
    base = Path(base)
    for fmt in FORMATS:
...
def plot_cp_normals(nodes, var_names, conn, base_out, cp,
    ...
    xi = _find_var_fuzzy(var_names, x_name)
    yi = _find_var_fuzzy(var_names, y_name)
    if xi is None or yi is None:
        raise ValueError("x/y nicht gefunden")
    x = nodes[:, xi].astype(float)
    y = nodes[:, yi].astype(float)
    order = (
        _concat_edges_to_order(conn.tolist(), len(x))
    ...
    for label, size in SIZES:
        ...
        plt.ylabel("y")
        if legend_text:
            plt.legend([legend_text], loc="best", frameon=True)
        plt.tight_layout()
        _save_current_figure(Path(base_out), label, dpi)
        plt.close()


def save_cp_curve_csv(nodes, var_names, cp, out_csv):
    xi = _find_var_fuzzy(var_names, "x")
    yi = _find_var_fuzzy(var_names, "y")
    if xi is None or yi is None:
        raise ValueError("x/y nicht gefunden")
    x = nodes[:, xi].astype(float)
    y = nodes[:, yi].astype(float)
    xmin, xmax = np.nanmin(x), np.nanmax(x)
    c = xmax - xmin if np.isfinite(xmin) and np.isfinite(xmax) else np.nan
...
def plot_cp_curve(nodes, var_names, cp, base_out, legend_text=None, dpi=200):
    xi = _find_var_fuzzy(var_names, "x")
    if xi is None:
        raise ValueError("x nicht gefunden")
    x = nodes[:, xi].astype(float)
    xmin, xmax = np.nanmin(x), np.nanmax(x)
    c = xmax - xmin if np.isfinite(xmin) and np.isfinite(xmax) else np.nan
    x_over_c = (x - xmin) / c if np.isfinite(c) and c > 0 else np.full_like(x, np.nan)
...
def main():
    ...
    ap.add_argument("--png-out", type=Path, required=True, help="Basis-Dateiname (Endung egal)")
    ap.add_argument("--cp-png-out", type=Path, default=None, help="Basis-Dateiname für Cp-Plot")
    ...
    ap.add_argument("--xname", type=str, default="x")
    ap.add_argument("--yname", type=str, default="y")
    ap.add_argument("--base", type=float, default=0.0)
    ap.add_argument("--extra", type=float, default=0.05)
    ...
    p_inf = atm.get("p_inf", np.nan)
    q_inf = atm.get("q_inf", np.nan)
    rho_inf = atm.get("rho_inf", np.nan)
    v_inf = atm.get("v_inf", np.nan)
    legend_text = _legend_string(v_inf, rho_inf, p_inf, q_inf) if args.legend == "on" else None

    # Cp
    cp = compute_cp_on_wall(
        nodes, var_names, p_inf, q_inf, p_name=args.p_name, cp_name=args.cp_name
    )

    # plots (multi-size, multi-format)
    plot_cp_normals(
        nodes,
        var_names,
        conn,
    ...
    if args.csv_out:
        save_cp_curve_csv(nodes, var_names, cp, args.csv_out)

    if args.cp_png_out:
        plot_cp_curve(nodes, var_names, cp, args.cp_png_out, legend_text=legend_text, dpi=args.dpi)

    # Info-Ausgabe
    print("Legend:\n", legend_text)
    base = Path(args.png_out)
    created_main = [base.with_name(base.stem + f"_{lbl}." + fmt) for lbl, _ in SIZES for fmt in FORMATS]
    print("Normalen-Plot Dateien:")
    for p in created_main:
        print(" ", p)
    if args.cp_png_out:
        base_cp = Path(args.cp_png_out)
        created_cp = [base_cp.with_name(base_cp.stem + f"_{lbl}." + fmt) for lbl, _ in SIZES for fmt in FORMATS]
        print("Cp-Plot Dateien:")
        for p in created_cp:
            print(" ", p)
    if args.csv_out:
        print("Cp-CSV:", args.csv_out)
