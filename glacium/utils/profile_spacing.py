...
fig, ax = plotter.new_figure(figsize=(10, 4))
ax.set_aspect("equal")

# original profile
plotter.line(ax, x, y, "k-", label="Profil")

# shifted contours
plotter.line(ax, x_up_s, y_up_s, "r-", label="Oberseite")
plotter.line(ax, x_lo_s, y_lo_s, "b-", label="Unterseite")
...
ax.legend()
ax.set_title("Funktion (linear bis 40% c, dann konstant) + Normal‑Verbindungen")
fig.show()
plotter.close(fig)
