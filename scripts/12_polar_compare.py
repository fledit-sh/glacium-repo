...
def plot_combined(
    ...
    # CL on left axis
    ln1 = ax_cl.plot(aoa_clean, cl_clean, color="dimgray", marker="^", linestyle="-", linewidth=0.8, label="CL clean")
    ln2 = ax_cl.plot(aoa_iced,  cl_iced, color="darkred", marker="^", linestyle="-", linewidth=0.8, label="CL iced")

    # CD on right axis (use different linestyle to distinguish quantity)
    ln3 = ax_cd.plot(aoa_clean, cd_clean, color="darkgrey", marker="v", linewidth=0.8, linestyle="--", label="CD clean")
    ln4 = ax_cd.plot(aoa_iced,  cd_iced,  color="red", marker="v", linewidth=0.8, linestyle="--", label="CD iced")
