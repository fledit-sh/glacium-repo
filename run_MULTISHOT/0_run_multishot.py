...
def main() -> None:
    ...
    # Nach der Shot-Verarbeitung erzeugen wir eine globale Mesh-Übersicht (nodes/shot)
    try:
        mesh_png = args.output_dir / "mesh_nodes_per_shot.png"
        mesh_csv = args.output_dir / "mesh_nodes_per_shot.csv"
        run_cmd(
            [
                sys.executable,
                str(SCRIPT_DIR / "04_mesh.py"),
        ...
        logging.info("Mesh-Übersicht erstellt: %s", mesh_png)
    except Exception as exc:
        logging.error("Mesh-Übersicht fehlgeschlagen: %s", exc)
